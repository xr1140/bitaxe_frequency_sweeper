...
def parse_arguments():
    ...
    parser.add_argument("-v", "--voltage", type=int, help="Core voltage in mV (minimum 1000 mV, required for monitor mode or single voltage tests)")
    parser.add_argument("-start", "--start_voltage", type=int, help="Starting core voltage in mV for voltage sweep (minimum 1000 mV)")
    parser.add_argument("-stop", "--stop_voltage", type=int, help="Ending core voltage in mV for voltage sweep (minimum 1000 mV)")
    parser.add_argument("-f", "--frequency", type=int, help="Initial frequency in MHz (minimum 400 MHz, required for monitor mode or single voltage tests without -values)")
    parser.add_argument("-ip", "--ip_address", type=str, required=True, help="Bitaxe IP address (e.g., 192.168.2.205)")
    parser.add_argument("-range", type=int, default=10, help="Frequency range in MHz to test above and below the initial or calculated frequency (default 10 MHz, ignored in monitor mode)")
    parser.add_argument("-step", type=int, default=2, help="Frequency step size in MHz (default 2 MHz, ignored in monitor mode)")
    parser.add_argument("-reboot", type=int, default=None, help="Number of consecutive identical hashrate readings to trigger a reboot (optional)")
    parser.add_argument("-m", "--monitor", action="store_true", help="Run in monitor-only mode at the specified voltage and frequency indefinitely (sets range=0, step=0)")
    parser.add_argument("-values", type=str, help="Path to values.csv file with voltage, frequency, and hashrate (used in monitor mode or with -start and -stop)")
